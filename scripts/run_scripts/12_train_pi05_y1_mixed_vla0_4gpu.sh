...
EXP_NAME="${EXP_NAME:-y1_dual_arm_mixed_pi05_vla0_4gpu_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "assets/$CONFIG_NAME/openpi/y1_dual_arm_mixed_20260526/norm_stats.json" ]]; then
  echo "Missing norm stats: assets/$CONFIG_NAME/openpi/y1_dual_arm_mixed_20260526/norm_stats.json" >&2
  echo "Run scripts/run_scripts/11_compute_norm_stats_y1_mixed.sh first." >&2
  exit 1
fi
if [[ ! -f "$HF_LEROBOT_HOME/openpi/y1_dual_arm_mixed_20260526/meta/info.json" ]]; then
  echo "Missing local LeRobot dataset: $HF_LEROBOT_HOME/openpi/y1_dual_arm_mixed_20260526" >&2
  exit 1
fi
